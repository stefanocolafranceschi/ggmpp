...
int main() {
    ...
    for ( jj; jj<=NumberOfRuns; jj++ ) {
        ...
        while (event<events) {
            ...
            //if (multiplereading) {
                ...
                if (!irqenabled) {
                    ...
                    dd = 100;
                    ret=CAENVME_WriteCycle(BHandle, (BoardAddress+0x00001020), &dd, cvA32_U_DATA, cvD16);
                    switch (ret) {
                        case cvSuccess   :
		...
		//PrintStatus(ret);
            ...
            for ( h = 0; h < EventsToRead; h++) {                                // Reading 32 Event with BLT
                ...
                if (bin[5]=='1' && bin[6]=='0' && bin[7]=='0' && starting==1) {     //this is end of block
                    ...
                    if (terminal) printf("%d %d %d %s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",event,event_buffer,now,buf,charge0,charge1,charge2,charge[3],charge[4],charge[5],charge[6],charge[7],charge[8],charge[9],charge[10],charge[11],charge[12],charge[13],charge[14],charge[15]);
                    if (rootfile) {
                        GGMraw->Fill();
                        //printf("filling");
                    }
